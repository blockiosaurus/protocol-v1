...
async function updateFundingRateHelper(
	...
	for (let i = 0; i < prices.length; i++) {
		...
		setFeedPrice(anchor.workspace.Switchboard, newprice, priceFeedAddress);
...
describe('switchboard-oracle', () => {
	...
	before(async () => {
		...
		const price = 50000;
		await mockSwitchboardOracle(price, -6);
	...
	it('change feed price', async () => {
		...
		const expo = -9;
		console.log("Here 0");
		const priceFeedAddress = await mockSwitchboardOracle(price, expo);
	...
	it('oracle/vamm: funding rate calc 0hour periodicity', async () => {
		const priceFeedAddress = await mockSwitchboardOracle(40, -10);
